    ...
    for tool_name in workflow["tools"]:
        try:
            if tool_name == "diagnose_anemia":
                ...
                results["diagnostic_results"].append({"tool": tool_name, "result": res})
                await _try_comparison(ctx, "mental_health", patient_id, results, fhir_server, fhir_token)
                ...
                results["diagnostic_results"].append({"tool": tool_name, "result": res})
                await _try_comparison(ctx, "parkinsons", patient_id, results, fhir_server, fhir_token)

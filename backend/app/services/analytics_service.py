...
class AnalyticsService:
    ...
    def get_user_analytics(
        ...
        previous_new_users = self.db.query(
            func.count(User.id)
        ).filter(
    ...
    def get_appointment_analytics(
        ...
        completed = next((count for status, count in appointments_by_status if status == 'completed'), 0)
        cancelled = next((count for status, count in appointments_by_status if status == 'cancelled'), 0)
        ...
        result = {
            ...
            'appointments_by_status': {status: count for status, count in appointments_by_status},
            'completed': completed,
            'cancelled': cancelled,
    ...
    def get_ai_usage_analytics(
        ...
        # Total consultations
        total_consultations = self.db.query(
            func.count(AIConsultation.id)
        ).filter(
        ...
        avg_confidence = self.db.query(
            func.avg(AIConsultation.confidence_score)
        ).filter(
            AIConsultation.created_at >= start_date,
            AIConsultation.created_at <= end_date
        ).scalar() or 0.0
        ...
        medium_confidence = self.db.query(
            func.count(AIConsultation.id)
        ).filter(
            AIConsultation.created_at >= start_date,
            AIConsultation.created_at <= end_date,
        ...
        for i in range(7):
            ...
            day_avg_confidence = self.db.query(
                func.avg(AIConsultation.confidence_score)
            ).filter(
                AIConsultation.created_at >= day_start,
                AIConsultation.created_at < day_end
    ...
    def get_revenue_analytics(
        ...
        completed_appointments = self.db.query(
            func.count(Appointment.id)
        ).filter(
            Appointment.created_at >= start_date,
            Appointment.created_at <= end_date,
        ...
        previous_period_start = start_date - (end_date - start_date)
        previous_completed = self.db.query(
            func.count(Appointment.id)
        ).filter(
        ...
        if previous_revenue > 0:
            revenue_growth = ((estimated_revenue - previous_revenue) / previous_revenue) * 100
    ...
    def get_messaging_analytics(
        ...
        active_conversations = self.db.query(
        ...
        for i in range(7):
            ...
            day_conversations = self.db.query(
                func.count(func.distinct(Message.conversation_id))
            ).filter(
                Message.created_at >= day_start,
                Message.created_at < day_end

...
def _init_sentry_safe():
    ...
    try:
        ...
        print("Sentry initialized (HttpxIntegration disabled for Supabase compatibility).")
...
@mcp.tool()
async def screen_dr_tool(ctx: Context, image_url: str, patient_id: Optional[str] = None) -> dict:
    return await screen_diabetic_retinopathy(ctx=ctx, image_url=image_url, patient_id=patient_id)
...
async def analyze_mental_health_tool(
    ...
    return await analyze_mental_health(ctx=ctx, audio_url=audio_url, patient_id=patient_id)
...
def create_app():
    ...
    fastapi_app = FastAPI(title="NetraAI MCP Bridge", version="2.0.0", lifespan=lifespan)
    ...
    async def rate_limit_middleware(request: Request, call_next):
        ...
        rate_limit_store[client_ip] = [
            ts for ts in rate_limit_store[client_ip] if now - ts < timedelta(seconds=RATE_WINDOW)
        ]
    ...
    async def call_tool(request: Request):
        ...
        try:
            ...
            tool_map = {
                ...
                "screen_dr_tool": lambda: screen_diabetic_retinopathy(ctx=None, **arguments),
                "analyze_mental_health_tool": lambda: analyze_mental_health(ctx=None, **arguments),
                "screen_parkinsons_tool": lambda: screen_parkinsons(ctx=None, **arguments),
                "get_patient_fhir_tool": lambda: get_patient_fhir(ctx=None, **arguments),
                ...
                "generate_prior_auth_tool": lambda: generate_prior_auth(ctx=None, **arguments),
            ...
            return JSONResponse({"error": "Internal server error", "tool": name}, status_code=500)
